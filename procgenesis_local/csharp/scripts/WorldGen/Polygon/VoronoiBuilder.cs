...
/// </summary>
internal sealed class SiteIndex
{
    ...
    {
        _width = width;
        _bucketSize = Math.Max(bucketSize, 1e-6d);
        _siteX = siteX;
        _siteY = siteY;
        _columns = Math.Max(1, (int)Math.Ceiling(width / _bucketSize));
        _rows = Math.Max(1, (int)Math.Ceiling(height / _bucketSize));
    ...
    {
        var column = WrapColumn((int)Math.Floor(x / _bucketSize));
        var row = ClampRow((int)Math.Floor(y / _bucketSize));
    ...
    /// </summary>
    public void CollectImages(int id, double referenceX, double referenceY, double radius, List<Candidate> output)
    {
    ...
    /// </summary>
    public void CollectWithin(double x, double y, double radius, List<Candidate> output)
    {
        var ringCount = (int)Math.Ceiling(radius / _bucketSize);
        var centerColumn = WrapColumn((int)Math.Floor(x / _bucketSize));
        var centerRow = ClampRow((int)Math.Floor(y / _bucketSize));
        var radiusSquared = radius * radius;

        for (var row = centerRow - ringCount; row <= centerRow + ringCount; row++)
        {
            if (row < 0 || row >= _rows)
            {
                continue;
            ...
            for (var column = centerColumn - ringCount; column <= centerColumn + ringCount; column++)
            {
    ...
    public int FindNearest(double x, double y, int excludeId, out double distance)
    {
        ...
        for (var ring = 0; ring <= maxRing; ring++)
        {
            ...
            // 停止判据必须与 bestDistanceSquared 同量纲：环 r 之外的桶里，
            // 任何点到查询点的距离都至少是 r × 桶边长。
            var ringReach = ring * _bucketSize;
