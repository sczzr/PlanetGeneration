...
public sealed class PolygonCivilizationResult
{
    ...
    /// <summary>纪元回放事件（最近若干纪元）。</summary>
    public required PolygonCivilizationEvent[] RecentEvents { get; init; }
}

/// <summary>一个纪元的文明事件，供叙事与回放使用。</summary>
public sealed class PolygonCivilizationEvent
{
...
public static class PolygonCivilizationSimulator
{
    ...
    public static PolygonCivilizationResult Simulate(
        PolygonGrid grid,
        int seed,
        int epoch,
        int civilAggression,
        ...
        float seaLevel)
    {
        ...
        return new PolygonCivilizationResult
        {
    ...
    /// </summary>
    private static bool[] BuildLandMask(PolygonGrid grid, float seaLevel)
    {
    ...
    /// </summary>
    private static bool IsAdjacentToOcean(PolygonGrid grid, int cell, bool[] landMask)
    {
    ...
    private static bool HasForeignNeighbor(PolygonGrid grid, int cell, short polityId, short[] polityIdMap)
    {
        var start = grid.CellNeighborStart[cell];
        var end = grid.CellNeighborStart[cell + 1];
        for (var k = start; k < end; k++)
    ...
    private static bool IsFarEnough(PolygonGrid grid, int cell, List<int> chosen, float spacing)
    {
    ...
    private static int FindLargestLandCell(PolygonGrid grid, bool[] landMask)
    {
        var best = -1;
        var bestArea = 0d;
        for (var cell = 0; cell < grid.Count; cell++)
    ...
    /// </summary>
    private static PolityArchetype DeterminePolityArchetype(PolygonGrid grid, int cell, float seaLevel)
    {
        var fields = grid.Fields;
    ...
    private static bool IsAdjacentToRiver(PolygonGrid grid, int cell)
    {
    ...
    private static bool IsAdjacentToOceanCell(PolygonGrid grid, int cell, float seaLevel)
    {
        var start = grid.CellNeighborStart[cell];
        var end = grid.CellNeighborStart[cell + 1];
        for (var k = start; k < end; k++)
    ...
    /// </summary>
    private static PolygonCivilizationEvent[] BuildEpochEventLog(
        int seed,
        int epoch,
        float aggressionNorm,
        float diversityNorm,
        ...
        float connectedHubPercent)
    {
        ...
        {
            return Array.Empty<PolygonCivilizationEvent>();
        }

        const int lookback = 6;
        var startEpoch = Math.Max(1, epoch - lookback + 1);
        var events = new List<PolygonCivilizationEvent>(lookback);
        ...
        for (var currentEpoch = startEpoch; currentEpoch <= epoch; currentEpoch++)
        {
            ...
            events.Add(new PolygonCivilizationEvent
            {
                Epoch = currentEpoch,
                Category = category,
                Summary = summary,
